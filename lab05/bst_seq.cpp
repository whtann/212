...
Node *insert(Node *T, int v, int r)
{
  ...
  int rankNode = T->left ? T->left->size : 0;
  if (r <= rankNode) { T->left = insert(T->left, v, r); }
  if (r > rankNode) { T->right = insert(T->right, v, (r - rankNode - 1)); }
  T->size++;
  return T;
}
...
Node *remove(Node *T, int r)
{
  ...
  // TBD: fill this part in
  int rankNode = T->left ? T->left->size : 0;
  if (r < rankNode) { T->left = remove(T->left, r); T->size--; }
  else if (r > rankNode) {T->right=remove(T->right,(r-rankNode-1)); T->size--;}
  else
  {
...
void split(Node *T, int r, Node **L, Node **R)
{
  ...
  {
    split(T->right, (r - rankNode - 1), &T->right, R);
    *L = T;
    if (*R) { T->size -= (*R)->size; }
  }
}
...
Node *insert_keep_balanced(Node *T, int v, int r)
{
  ...
  {
    int rankNode = T->left ? T->left->size : 0;
    if (r <= rankNode) { T->left = insert_keep_balanced(T->left, v, r); }
    if (r > rankNode)
    { T->right = insert_keep_balanced(T->right, v, (r - rankNode - 1)); }
    T->size++;
    return T;
  }
}
...
Node *order_n_teams(int n)
{
  ...
  for (int i=1; i<n; i++) {
	...
	T = insert(T, i, T->size); // can we insert at end?
        ...
        int max = (T->size - 1); int min = 0; int middle = 0;
        while (min != (max - 1))
        {
          middle = (max + min) / 2;
          if (did_x_beat_y(i, select(T, middle)->key)) { max = middle; }
          else { min = middle; }
        }
        T = insert(T, i, max);
      }
